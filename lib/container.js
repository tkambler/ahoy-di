...
class Container {
    ...
    getMappedService(name) {
        ...
        const containerPath = name.split('.');
        const serviceName = containerPath.pop();
        const containerPathRev = containerPath.reverse();
        
        debug('getMappedService', {
            'name': name,
            'containerPath': containerPath,
            'containerPathRev': containerPathRev,
            'serviceName': serviceName
        });
        
        let targetContainer;
        let tmpContainer = this;
        let mappedService;
        const tmpPaths = [];
        
        if (this.children[containerPath[0]]) {

            for (let k in containerPathRev) {
                k = parseInt(k, 10);
                const containerId = containerPathRev[k];
                debug('Looking for child container.', {
                    'id': containerId,
                    'children': tmpContainer.children,
                    'tmpPaths': tmpPaths,
                    'k': k
                });
                if (!tmpContainer.children[containerId]) {
                    break;
                }
                tmpPaths.push(containerId);
                tmpContainer = tmpContainer.children[containerId];
                if ((k + 1) === containerPathRev.length) {
                    mappedService = tmpContainer.fetch(serviceName);
                }
            }
            ...
            for (let k in containerPathRev) {
                k = parseInt(k, 10);
                const containerId = containerPathRev[k];
                debug('Looking for parent container.', {
                    'id': containerId,
                    'parent': tmpContainer.parent,
                    'tmpPaths': tmpPaths,
                    'k': k
                });
                if (!tmpContainer.parent) {
                    break;
                }
                tmpPaths.push(tmpContainer.parent.id);
                if (this.parent.id !== containerId) {
                    break;
                }
                tmpContainer = this.parent;
                if ((k + 1) === containerPathRev.length) {
                    mappedService = tmpContainer.fetch(serviceName);
                }
            }
